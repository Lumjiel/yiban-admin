...
def generate_user_data_content() -> str:
    ...
    if notify:
        ...
        mail_config_str += f'    "qq": "{sanitize_str(notify["qq"], 20)}",\n'
        mail_config_str += f'    "auth_code": "{sanitize_str(notify["auth_code"], 50)}"\n'
    else:
    ...
    for u in users:
        if not u['enable']:
            continue
        user_data_str += "    {\n"
        user_data_str += f"        'Phone': '{sanitize_str(u['phone'], 11)}',\n"
        user_data_str += f"        'QQ': '{sanitize_str(u['qq'], 20)}',\n"
        user_data_str += f"        'PassWord': '{sanitize_str(u['password'], 50)}',\n"
        user_data_str += f"        'SendKey': '{sanitize_str(u['sendkey'] or '', 100)}',\n"
        user_data_str += f"        'Address': '{sanitize_str(u['address'], 200)}',\n"
        user_data_str += f"        'DeviceID': '{sanitize_str(u['device_id'], 50)}',\n"
        user_data_str += f"        'PhoneModel': '{sanitize_str(u['phone_model'], 50)}',\n"
...
def sync_to_server() -> tuple:
    ...
    try:
        proc = subprocess.run(
            ["scp", "-P", "22222", "-",
             f"root@49.234.178.53:{SERVER_PATH}/yiban/config/user_data.py"],
            input=content.encode(), capture_output=True, timeout=15,
        )
        if proc.returncode == 0:
            return True, "同步成功"
        else:
            return False, f"同步失败: {proc.stderr.decode()}"
...
def trigger_sign_stream(phone: Optional[str] = None, test_mode: bool = False) -> Generator:
    ...
    try:
        proc = subprocess.Popen(
            ["ssh", "-p", "22222", "root@49.234.178.53", cmd],
