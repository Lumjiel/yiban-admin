...
# 兜底链路：每天 08:00 检查当天是否有签到记录，没有则 ServerChan 告警
0 8 * * * cd /opt/yiban-admin && python3 check_daily.py >> /var/log/yiban-check.log 2>&1
```

**为什么需要两条？** 主链路的失败通知只在"脚本成功运行了"的前提下才会发出。
服务器停机、脚本崩溃、cron 未触发等场景下，失败通知本身也发不出去——这就是静默失败。

兜底链路是一个独立的旁观者：不依赖签到脚本存活，只查结果数据库。
当天无任何记录（含 skip）→ 推送「⚠️ 易班签到未执行」；有记录 → 静默不打扰。

| 场景 | 主链路通知 | 08:00 兜底 |
|------|-----------|-----------|
| 签到成功 | 汇总通知 | 静默 |
| 签到失败 | A/B 类失败通知 | 静默（有记录即视为执行过） |
| 全部跳过（周日等） | 跳过汇总通知 | 静默 |
| **停机 / 崩溃 / cron 未触发** | ❌ 发不出 | ✅ **推送告警** |
